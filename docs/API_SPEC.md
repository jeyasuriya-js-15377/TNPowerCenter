...
Base path: `/server/tnpc_api` (the API function).
...
|---|---|---|---|
| `GET` | `/health` | — | Liveness; reports whether portal credentials are present |
...
| `POST` | `/complaints` | `complaint:read` | Citizen intake → classify → route → create a complaint record |
...
| `POST` | `/directives` | `directive:issue` | **Privileged.** Creates the record *and* an execution task |
...
department's workspace — a directive is an executable object, not a document.
...
```json
"freshness": {
  "source": "the project portal", "lastUpdated": "…", "state": "FRESH",
  "dataGaps": [],
